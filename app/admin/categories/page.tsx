...
export default function AdminCategoriesPage() {
  ...
  const handleDelete = (cat: Category) => {
    ...
    if (!success) {
      toast.error(
        `Cannot delete "${cat.name}" because products are currently assigned to it. Reassign products first.`
      );
    } else {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 md:p-6 rounded-2xl border border-gray-200/80 shadow-2xs">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Organize tiles into room and application categories
          </p>
        </div>
                ...
                <div className="p-4 space-y-2">
                  ...
                  <div className="flex items-center gap-2 pt-2 text-xs">
                    <span className="font-bold text-[#052a51] px-2 py-0.5 bg-gray-100 rounded-md">
                      {liveProductCount} Active Tile(s)
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 pt-0 flex items-center justify-between border-t border-gray-100 mt-2">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="p-1.5 text-gray-400 hover:text-[#052a51] hover:bg-gray-100 rounded-lg"
                    title="Edit category"
                  >
                    <Edit size={14} />
                  </button>
