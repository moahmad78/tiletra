...
export default function ConnectDashboard({
  ...
  return (
    <div className="w-full flex-1 flex flex-col bg-[#071321] text-slate-100 font-sans min-h-[calc(100vh-64px)]">
      {/* Top Header Bar */}
      <header className="h-16 bg-[#0a192f] border-b border-cyan-900/40 px-4 md:px-6 flex items-center justify-between z-20 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-full text-[10px] font-mono bg-cyan-950 text-cyan-300 border border-cyan-800/50">
                {portalContext === "admin" ? "/admin/connect" : "/help"}
              </span>
            ...
            <p className="text-xs text-slate-400">Customer Support Desk & AI Communication OS</p>
          </div>
        </div>
        ...
        {/* Authenticated User Status, Links & Logout */}
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-900/90 border border-slate-800 text-xs text-slate-300">
            ...
            <span className="font-mono text-cyan-300 font-medium">{currentAgentEmail}</span>
            <span className="text-slate-500">|</span>
            <span className="text-[11px] text-slate-400">
              {portalContext === "admin" ? "Admin Desk" : "Authorized Lead"}
            </span>
          </div>

          {/* Quick link to /help if in admin */}
          {portalContext === "admin" && (
            <a
              href="/help"
              target="_blank"
              rel="noopener noreferrer"
              title="Open standalone /help desk portal"
              className="hidden lg:flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-800/80 hover:bg-slate-700 text-cyan-300 text-xs border border-slate-700 font-medium transition-colors"
            >
          ...
          {showLogout && onLogout && (
            <button
              ...
              className="px-2.5 py-1.5 rounded-lg bg-red-950/40 hover:bg-red-900/40 text-red-300 border border-red-800/40 text-xs font-medium flex items-center gap-1.5 transition-colors"
            >
      ...
      <main className="flex-1 flex overflow-hidden">
        {activeTab === "inbox" && (
          <div className="w-full flex flex-col lg:flex-row flex-1 overflow-hidden">
            {/* COLUMN 1: Unified Conversation List */}
            ...
            {/* COLUMN 2: Message Thread & AI Suggested Reply Workspace */}
              ...
              {selectedConversation && (
                <div className="h-16 px-4 md:px-6 bg-[#0a1829] border-b border-slate-800/80 flex items-center justify-between shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2.5 py-1 rounded-lg bg-slate-800 border border-slate-700 text-xs text-cyan-300 font-mono">
                      {selectedConversation.assignedTo || currentAgentEmail}
                    </span>
                  </div>
                </div>
              )}
                  ...
                  {loadingAi ? (
                    ...
                    <div className="space-y-3">
                      <div className="px-3 py-1.5 rounded-lg bg-emerald-950/40 border border-emerald-800/40 text-[11px] text-emerald-300 flex items-center gap-2">
                        ...
                        <span>{aiSuggestion.guardrailNotes || "Verified strictly against approved IntriHub sources."}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-2 pt-2">
                        ...
                        <button
                          onClick={() => {
                            setAgentDraftText(aiSuggestion.suggestedReply);
                            const composer = document.getElementById("connect-composer-input");
                            composer?.focus();
                          }}
            ...
            {/* COLUMN 3: Customer 360 & Live Context */}
