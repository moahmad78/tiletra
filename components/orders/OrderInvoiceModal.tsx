...
export default function OrderInvoiceModal({
  ...
  const orderId = order?.orderNumber || (order?.id || "").replace("IH-", "").replace("TL-", "").replace("ord_", "").toUpperCase() || "ORD-0000";
  ...
  const address =
    typeof order.shippingAddress === "string"
      ...
      : order.shippingAddress
      ? [
          order.shippingAddress.houseNumber ? `House/Flat: ${order.shippingAddress.houseNumber}` : "",
          order.shippingAddress.buildingName ? `Building: ${order.shippingAddress.buildingName}` : "",
          ...
          order.shippingAddress.pincode ? `Pincode: ${order.shippingAddress.pincode}` : "",
        ]
          .filter(Boolean)
          .join(", ")
      : "Begur Main Road, Bangalore, Karnataka - 560114";
                ...
                <tbody className="divide-y divide-gray-100 font-medium">
                  {order.items?.map((item: any, idx: number) => (
                    <tr key={idx}>
                      ...
                      <td className="py-3 px-2 text-center font-bold">{item.boxQuantity || item.quantity || 1} Box</td>
