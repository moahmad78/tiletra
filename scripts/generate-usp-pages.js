...
// Read USP data from lib/data/usps.ts by extracting definitions
const tsContent = fs.readFileSync(path.join(__dirname, "../lib/data/usps.ts"), "utf8");

// Parse USP_ITEMS roughly or compile with ts-node/require
// Since usps.ts is TypeScript, let's extract the slugs and data
const slugsMatch = [...tsContent.matchAll(/slug:\s*"([^"]+)"/g)].map(m => m[1]);

console.log(`Found ${slugsMatch.length} USP slugs:`, slugsMatch);

slugsMatch.forEach((slug) => {
  const pageDir = path.join(__dirname, `../app/${slug}`);
...
`;
  ...
  console.log(`Generated app/${slug}/page.tsx`);
});
console.log("All 20 USP pages generated successfully!");
