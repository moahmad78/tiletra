...
async function runQaSuite() {
  ...
  assert(allDbPages.length === 50, "All 50 SEO landing pages exist in database", `Found ${allDbPages.length}`);
  ...
  assert(wordCountFails === 0, "All 50 pages have >= 300 words of introContent", `${wordCountFails} failed`);
  ...
  assert(napFails === 0, "NAP block identical and compliant across all 50 generated schemas", `${napFails} mismatches`);
  ...
  assert(missingPrimarySlugs === 0, "Sitemap contains all 50 primary SEO landing slugs", `${missingPrimarySlugs} missing`);
  ...
  assert(blockedLandingSlugs === 0, "Robots.txt permits crawling of all 50 SEO landing routes", `${blockedLandingSlugs} blocked`);
  assert(
